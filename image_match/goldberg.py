...
class ImageSignature(object):
    ...
    def __init__(
        ...
        Args:
            ...
            fix_ratio (Optional[boolean]): use the larger crop ratio for both directions. Only use
                if you can guarantee the incoming image is square (default False)
    ...
    def generate_signature(self, path_or_image: ImageInput, bytestream: bool = False) -> np.ndarray:
        ...
        if self.crop_percentiles is not None:
            image_limits = self.crop_image(im_array, lower_percentile=self.lower_percentile, upper_percentile=self.upper_percentile, fix_ratio=self.fix_ratio)
        else:
            image_limits = None
        ...
        diff_mat = self.compute_differentials(avg_grey, diagonal_neighbors=self.diagonal_neighbors)

        # Step 4b: Bin differences to only 2n+1 values
        self.normalize_and_threshold(diff_mat, identical_tolerance=self.identical_tolerance, n_levels=self.n_levels)
    ...
    def crop_image(image: np.ndarray, lower_percentile: int = 5, upper_percentile: int = 95, fix_ratio: bool = False) -> list[tuple[int, int]]:
        ...
        if fix_ratio:
            if (upper_row_limit - lower_row_limit) > (upper_column_limit - lower_column_limit):
                return [(lower_row_limit, upper_row_limit), (lower_row_limit, upper_row_limit)]
            else:
                return [(lower_column_limit, upper_column_limit), (lower_column_limit, upper_column_limit)]
    ...
    def normalize_and_threshold(difference_array: np.ndarray, identical_tolerance: float = 2 / 255.0, n_levels: int = 2) -> None:
        ...
        positive_differences = difference_array[difference_array > 0.0]
        negative_differences = difference_array[difference_array < 0.0]

        positive_cutoffs = np.percentile(positive_differences, np.linspace(0, 100, n_levels + 1)) if positive_differences.size else None
        negative_cutoffs = np.percentile(negative_differences, np.linspace(100, 0, n_levels + 1)) if negative_differences.size else None

        if positive_cutoffs is not None:
        ...
        if negative_cutoffs is not None:
